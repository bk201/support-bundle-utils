...
BUNDLE_DIR="${OUTPUT_DIR}/bundle-${NODE_NAME}"
...
tail -c 100M /var/log/k3s-service.log > k3s-service.log
tail -c 100M /var/log/k3s-restarter.log > k3s-restarter.log
...
set -o errexit
curl -v -i -H "Content-Type: application/zip" --data-binary @bundle.zip "${HARVESTER_SUPPORT_BUNDLE_MANAGER_URL}/nodes/${NODE_NAME}"
